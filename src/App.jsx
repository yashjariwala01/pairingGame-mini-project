...
const MemoryGame = () => {
  ...
  setTimeout(()=>{
    let allMatched = matchedCards.length === cards.length;
  ...
  return (
    <div className="flex flex-col items-center gap-4 pt-3 memory-game bg-gradient-to-r from-red-300 via-red-500 to-purple-400 h-[100vh]">
      ...
      <div className={`card-grid border-8 border-green-200 rounded-xl border-black border-solid p-2 ${allMatched ? 'invisible absolute':''}`}>
        {cards.map((fruits, index) => <RenderCard key={index} index={index} fruits={fruits} flippedCards={flippedCards} matchedCards={matchedCards} handleCardClick={handleCardClick}/>)}
      </div>
...
// Render the card
const RenderCard = ({index,fruits,flippedCards,matchedCards, handleCardClick}) => {
